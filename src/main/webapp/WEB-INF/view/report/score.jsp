...
<script>
    ...
    var socket = new WebSocket("ws://localhost:8080/UniversityAdmission/score");
    socket.onopen = function () {
        // alert('Connected successfuly to websocket');
        // document.getElementById("connection").innerHTML = "Connected successfuly";
    };

    socket.onclose = function (event) {
        // if (event.wasClean) {
        //     // alert('Соединение закрыто чисто');
        //     alert('Connection closed websocket');
    ...
    socket.onmessage = function (event) {
        ...
        <%--facultyInfo[0].innerHTML = scoreChanges.faculty.nameEng;--%>
        <%--<c:out value="${score.faculty.nameEng}"></c:out>--%>
        <%--</c:when>--%>
        <%--<c:otherwise>--%>
        <%--facultyInfo[0].innerHTML = scoreChanges.faculty.nameRu;--%>
        <%--</c:otherwise>--%>
        <%--</c:choose>--%>
