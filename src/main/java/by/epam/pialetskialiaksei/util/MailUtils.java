...
public class MailUtils {
    ...
    public static String sendConfirmationEmail(Client client, User user) throws MessagingException, UnsupportedEncodingException, CommandException {

        String token = new RandomString().nextString();
        MailUtils.sendEmail(client, user, token);

        return token;
    }


    private static void setContentToConfirmationEmailEng(Message msg, Client client, User user, String token)
        ...
        String hello = "Hello, " + client.getFirstName() + " "
                + client.getLastName() + " !\n"
    ...
    private static MimeMessage emailMessage(Client client, User user, String token) throws MessagingException, UnsupportedEncodingException {
        MimeMessage emailMessage = new MimeMessage(SESSION);
        emailMessage.setFrom(new InternetAddress(
                "mail.university.admission@gmail.com"));
        emailMessage.addRecipient(Message.RecipientType.TO,
                new InternetAddress(user.getEmail()));
        setContentToConfirmationEmailEng(emailMessage, client, user, token);
        emailMessage.setSentDate(new Date());
        return emailMessage;
    }

    private static void sendEmail(Client client, User user, String token) throws MessagingException, UnsupportedEncodingException, CommandException {
        String fromUser;
        ...
        MimeMessage emailMessage = emailMessage(client, user, token);
